...
- **I1 — TV canvas frozen.** No changes to `.display-panel`/`.displayPanel`, `.transcript-viewport/-stack/-item/-text` rules, the `--font-size`/`--display-margin` pipeline, `role="log" aria-live="polite"`, or card animations — EXCEPT the sanctioned tweaks: inactive-card opacity `.62 → .8` (task 016), never tightening `letter-spacing` below the current `-0.045em`, and (user-requested mid-execution) `.transcript-item` gets `overflow: hidden` + squared `::before` accent strip so the strip clips to the card's rounded silhouette.
