...
The app is a tiny local Express-served static UI. This plan does not change the server, source registry, provider adapters, or five-line display. It changes how existing helper controls are grouped so the operator sees fewer competing choices during a meeting.
...
| --- | --- | --- | --- |
| P1 — TV display surface | Shows exactly five viewer-facing lines, no labels. | `public/index.html`, `public/style.css`, `public/controller/view.js` | existing |
| P2 — Primary helper controls | Controls the live meeting flow: modes, manual line, start/stop/pause, undo, clear, viewer size/margins, fullscreen. | `public/index.html`, `public/style.css`, `public/controller/start-app.js`, `public/controller/runtime.js`, `public/controller/view.js` | changed |
| P3 — Settings disclosure | Houses lower-frequency configuration: transcription source, summary source, and summary update interval. | `public/index.html`, `public/style.css`, controller DOM queries | new/changed |
| P4 — Diagnostics disclosure | Houses lower-frequency runtime feedback: status and recent transcript. | `public/index.html`, `public/style.css`, `public/controller/view.js` | new/changed |
...
| --- | --- | --- | --- |
| P2 | P1 | Helper actions mutate runtime state and call view renderers. | Manual lines still render immediately; display remains exactly five lines with no labels. |
| P2 | P3 | Controller finds controls by ids and selectors such as `.mode`, `[data-kind="transcription"]`, `[data-kind="summarization"]`, and `[data-interval]`. | Moving controls into Settings must not remove ids/classes/data attributes used by existing JS. |
| P3 | Runtime state | Source and interval buttons update `transcriptionSource`, `summarizationSource`, and `summaryIntervalSeconds`. | Existing localStorage keys and source ids remain stable: `browser`, `openai`, `claude`. |
...
- **INV-1** — The TV display always shows exactly five visible lines, newest at the bottom, with no labels.
...
- **INV-4** — Source and interval choices remain keyboard/click operable and persist through current localStorage keys.
