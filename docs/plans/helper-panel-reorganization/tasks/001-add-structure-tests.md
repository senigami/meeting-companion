...
Add tests that fail until the helper panel has Settings and Diagnostics disclosure regions with the right controls inside them.
...
- `public/controller/start-app.js` queries controls by id and by selectors: `.mode`, `[data-kind="transcription"]`, `[data-kind="summarization"]`, and `[data-interval]`.
...
- Transcription source, summary source, and summary interval controls are inside Settings.
...
- Primary live controls remain outside Settings and Diagnostics: mode buttons, manual entry, start/stop/pause, undo, clear, bigger/smaller, display margin, font size, fullscreen.
