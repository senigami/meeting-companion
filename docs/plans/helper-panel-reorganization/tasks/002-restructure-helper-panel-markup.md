...
- The helper panel currently has sections for viewer options, update interval, mode, transcription source, summary source, manual line, paste transcript, button grid, status, and transcript.
- Source and interval buttons must keep their `data-kind`, `data-source`, and `data-interval` attributes.
...
- [ ] Settings contains transcription source, summary source, and interval controls.
...
- [ ] The display still has exactly five line elements.
- [ ] Targeted tests pass.
