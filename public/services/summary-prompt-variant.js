...
export function buildSummarizePromptVariant({
  mode = 'speaker',
  recentTranscript = '',
  previousBlock = '',
  visibleLines = [],
  ...
  const isNarrativeMode = mode === 'speaker' || mode === 'prayer';
...
Only return an empty string if the new transcript repeats what a visible line already says, or contains no words at all. Never return an empty string because what was said seems unimportant, small, or ordinary. If it is new, compress it and return it.
...
Aim for about ${wordLimit} words. Going one or two words over is fine and is much better than losing meaning. Never drop or soften a name, date, time, hymn number, scripture reference, or assignment to hit the number, and never cut the end of a line off to make it fit. If it will not fit, use shorter and simpler words rather than fewer facts.
