...
// Issue #52 (2026-08-04): the speaker label had no font-size of its own, so it inherited
// .transcript-meta's clamp() -- the OPERATOR CHROME scale -- and measured 12.48px against 84px card
// text. Ansel's ruling gave it its own scale, 0.4 of the card text with an absolute floor.
//
// Reversed 2026-08-17, Steve's direct call, made after that ruling and its tradeoff were stated
// plainly first: back to inheriting .transcript-meta's chrome scale, same as the mode label beside
// it. This test now pins THAT -- if it needs reverting again, the numbers this used to assert are in
// the git history of this file, not restated here.
test('the speaker label inherits the chrome scale, same as the mode label beside it', async () => {
  const css = await readSplitCss();
  ...
  assert.doesNotMatch(rule, /font-size:/,
    'it must NOT set its own size -- Steve\'s 2026-08-17 call was to go back to inheriting the chrome scale');
});
